...
def get_model(point_cloud, is_training, bn_decay=None):
    ...
    with tf.variable_scope('v', reuse=tf.AUTO_REUSE):
        ...
        l1_xyz, l1_points, l1_indices = pointnet_sa_module(point_cloud_transformed, l0_points, npoint=512, radius=0.1, nsample=16, mlp=[64], mlp2=None, group_all=False, is_training=is_training, bn_decay=bn_decay, scope='layer1')
        l0_points = pointnet_fp_module(point_cloud_transformed, l1_xyz, l0_points, l1_points, [64], is_training, bn_decay, scope='fa_layer1')
        l0_points = tf.expand_dims(l0_points, axis=2)
        print('l0_points:', l0_points.shape)
        net = net + l0_points
        print(net.shape)
        net1 = net
        l1_points = tf.squeeze(net1, [2])
        ...
        net = tf.reduce_max(net, axis=-2, keep_dims=True)
        l2_xyz, l2_points, l2_indices = pointnet_sa_module(point_cloud_transformed, l1_points, npoint=512, radius=0.2, nsample=32, mlp=[64], mlp2=None, group_all=False, is_training=is_training, bn_decay=bn_decay, scope='layer2')
        l0_points = pointnet_fp_module(point_cloud_transformed, l2_xyz, l1_points, l2_points, [64], is_training, bn_decay, scope='fa_layer2')
        l0_points = tf.expand_dims(l0_points, axis=2)
        net = net + l0_points
        net2 = net
        l2_points = tf.squeeze(net2, [2])
        adj_matrix = tf_util.pairwise_distance(net)
        nn_idx = tf_util.knn(adj_matrix, k=k)
        ...
        l3_xyz, l3_points, l3_indices = pointnet_sa_module(point_cloud_transformed, l2_points, npoint=512, radius=0.4, nsample=64, mlp=[64], mlp2=None, group_all=False, is_training=is_training, bn_decay=bn_decay, scope='layer3')
        l0_points = pointnet_fp_module(point_cloud_transformed, l3_xyz, l2_points, l3_points, [64], is_training, bn_decay, scope='fa_layer3')
        l0_points = tf.expand_dims(l0_points, axis=2)
        net = net + l0_points
        net3 = net
        l3_points = tf.squeeze(net3, [2])
        adj_matrix = tf_util.pairwise_distance(net)
        nn_idx = tf_util.knn(adj_matrix, k=k)
        ...
        l4_xyz, l4_points, l4_indices = pointnet_sa_module(point_cloud_transformed, l3_points, npoint=512, radius=0.8, nsample=128, mlp=[128], mlp2=None, group_all=False, is_training=is_training, bn_decay=bn_decay, scope='layer4')
        l0_points = pointnet_fp_module(point_cloud_transformed, l4_xyz, l3_points, l4_points, [128], is_training, bn_decay, scope='fa_layer4')
        l0_points = tf.expand_dims(l0_points, axis=2)
        net = net + l0_points
        net4 = net
